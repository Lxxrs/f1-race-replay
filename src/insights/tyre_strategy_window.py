...
class StintBar(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.position:
            ...
            painter.drawText(
                QRect(0, 6, POS_W - 4, H - 12), Qt.AlignCenter, str(self.position)
            )
        ...
        painter.drawText(
            QRect(POS_W, 0, NAME_W - 4, H), Qt.AlignVCenter | Qt.AlignLeft, self.code
        )
        ...
        for stint in self.stints:
            ...
            if bw > 14:
                text_col = (
                    QColor("#000000") if tyre in (2.0, 3.0) else QColor("#ffffff")
                )
                painter.setPen(text_col)
                painter.setFont(QFont("Arial", 8, QFont.Bold))
                label_w = min(bw - 4, 18)
                painter.drawText(
                    QRect(x1 + 3, 10, label_w, H - 20),
...
class TyreStrategyWindow(PitWallWindow):
    def __init__(self):
        ...
        self.statusBar().hide()
        self.setStyleSheet(
            "QMainWindow { background: #0f0f0f; } QStatusBar { background: #111111; }"
        )
    ...
    def setup_ui(self):
        ...
        header.setStyleSheet(
            "background:#111111; border-bottom:1px solid #2a2a2a; color:#ffffff;"
        )
        ...
        legend_row.setStyleSheet(
            f"background:#111111; border-bottom:1px solid {BORDER};"
        )
        ...
        status_bar.setStyleSheet(f"background:#0a0a0a; border-top:1px solid {BORDER};")
    ...
    def on_telemetry_data(self, data):
        ...
        for code, driver in drivers.items():
            ...
            if (
                tyre is None
                or lap is None
                or not isinstance(tyre, (int, float))
                or tyre == 0.0
            ):
                continue
            ...
            if code not in self.stints:
                ...
                self.stints[code].append(
                    {"tyre": tyre, "start_lap": lap, "end_lap": None}
                )
                self.prev_tyres[code] = tyre
    ...
    def _flush_redraw(self):
        ...
        sorted_codes = sorted(
            self.stints.keys(), key=lambda c: (self.positions.get(c, 999), c)
        )
        ...
        for i, code in enumerate(sorted_codes):
            ...
            if code not in self._row_widgets:
                bar = StintBar(
                    code, self.stints[code], self.total_laps, pos, self.current_lap
                )
