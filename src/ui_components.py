...
class LeaderboardComponent(BaseComponent):
    def __init__(self, x: int, right_margin: int = 260, width: int = 240, visible=True):
        ...
        self._gap_text = arcade.Text(
            "", 0, 0, arcade.color.LIGHT_GRAY, 12, anchor_x="right", anchor_y="top"
        )
        self._tyre_textures = {}
        self._visible: bool = visible
    ...
    def draw(self, window):
        ...
        self.neighbor_toggle_rect = (
            neighbor_x - toggle_radius,
        ...
        for i, (code, color, pos, progress_m) in enumerate(new_entries):
            ...
            if getattr(self, "show_neighbor_gaps", False) or getattr(
                self, "show_gaps", False
            ):
                gap_x = right_x - 36
            ...
            if tyre_texture:
                ...
                try:
                    life_display = (
                        str(int(current_life)) if pd.notna(current_life) else "0"
                    )
            ...
            try:
                ...
                if health_data:
                    ...
                    if bar_params["fill_width"] > 0:
                        arcade.draw_rect_filled(
                            arcade.XYWH(
                                bar_x + bar_params["fill_width"] / 2,
                                bar_y,
                                bar_params["fill_width"],
                                bar_params["height"],
                            ),
                            bar_params["color"],
                        )
