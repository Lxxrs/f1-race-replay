...
class TyreDegradationIntegrator:
    ...
    def initialize_from_session(self) -> bool:
        ...
        try:
            ...
            print(
                f"BayesianModel: Fitting state-space model on {len(self._laps_df)} laps..."
            )
            ...
            print(
                "BayesianModel: Degradation rates (seconds/lap) (If a set of tyres were not used in the race, the deg value denoted is the prior assumed in the model):"
            )
            for compound_name, tyre in self._model.tyre_profiles.items():
                print(
                    f"  {compound_name} ({tyre.category.value}): {tyre.degradation_rate:.4f}"
                )
    ...
    def get_tyre_health(
        ...
        try:
            health_data = self._model.get_health(
                driver_code, current_lap, self._laps_df, track_condition
            )
    ...
    def get_health_for_frame(
        self, driver_code: str, frame_data: Dict, frame_index: Optional[int] = None
    ) -> Optional[Dict]:
