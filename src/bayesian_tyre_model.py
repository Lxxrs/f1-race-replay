...
class TyreProfile:
    ...
    def __post_init__(self):
        if self.degradation_rate < 0:
            raise ValueError(
                f"Degradation rate must be non-negative: {self.degradation_rate}"
            )
...
class BayesianTyreDegradationModel:
    ...
    def estimate_track_abrasion(self, laps_df: pd.DataFrame) -> float:
        ...
        for compound, base_rate in baseline.items():
            slick_laps = laps_df[
                (laps_df["Compound"] == compound) & (laps_df["TrackCondition"] == "DRY")
            ]
                ...
                for stint in driver_laps["Stint"].unique():
                    ...
                    if delta.std() > 0:
                        slope, _, _, _ = stats.theilslopes(
                            delta.values, stint_laps["LapOnTyre"].values
                        )
        ...
        if self.config.debug_logging:
            ...
            print(
                f"  Track abrasion: {abrasion:.3f} ({track_type}, from {len(abrasion_samples)} stints)"
            )
    ...
    def _prepare_data(self, laps_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if invalid_conditions:
            print(
                f"Warning: Invalid track conditions found: {invalid_conditions}, setting to DRY"
            )
            laps.loc[
                ~laps["TrackCondition"].isin(valid_conditions), "TrackCondition"
            ] = "DRY"
    ...
    def _should_use_lap_for_fitting(self, compound: str, track_condition: str) -> bool:
    ...
    def _estimate_parameters(self, laps_df: pd.DataFrame):
                ...
                for stint in driver_laps["Stint"].unique():
                    ...
                    valid_laps = stint_laps[
                        stint_laps.apply(
                            lambda row: self._should_use_lap_for_fitting(
                                row["Compound"], row["TrackCondition"]
                            ),
                            axis=1,
                        )
                    ]
                    ...
                    if len(analysis_laps) > 2:
                        ...
                        if len(x) > 0 and np.std(y) > 0:
                            ...
                            if (
                                self.config.enable_track_abrasion
                                and self.track_abrasion > 0
                            ):
                                slope = slope / self.track_abrasion

                            if tyre.category == TyreCategory.INTER:
                                dry_fraction = (
                                    valid_laps["TrackCondition"] == "DRY"
                                ).sum() / len(valid_laps)
                                if dry_fraction > 0.3:
        ...
        for compound_name, tyre in self.tyre_profiles.items():
            if len(compound_slopes[compound_name]) > 0:
                ...
                print(
                    f"  {compound_name}: {tyre.degradation_rate:.4f} s/lap "
    ...
    def _compute_mismatch_penalty(self, compound: str, track_condition: str) -> float:
        ...
        return self.config.mismatch_penalties.get((tyre_category, condition), 0.0)
    ...
    def _compute_latent_states(self, laps_df: pd.DataFrame):
        ...
        for driver in laps_df["Driver"].unique():
            ...
            for _, lap in driver_laps.iterrows():
                ...
                if compound not in self.tyre_profiles:
                    print(
                        f"Warning: Unknown compound '{compound}' for {driver}, carrying forward state"
                    )
                ...
                current_condition = condition_map.get(
                    track_condition, TrackCondition.DRY
                )

                condition_category = (
                    "DRY" if current_condition == TrackCondition.DRY else "WET"
                )

                condition_changed = (
                    prev_condition_category is not None
                    and prev_condition_category != condition_category
                )
                ...
                if mu_alpha is None or stint != prev_stint or condition_changed:
                    if condition_changed and self.config.debug_logging:
                        ...
                        print(
                            f"  {driver}: Track transition {old_category}→{condition_category}, resetting pace"
                        )
                    ...
                    expected_lap = (
                        mu_pred_temp + self.fuel_effect * fuel + mismatch_penalty
                    )
    ...
    def predict_next_lap(
        ...
        driver_laps = laps_df[
            (laps_df["Driver"] == driver) & (laps_df["LapNumber"] <= current_lap)
        ].sort_values("LapNumber")
        ...
        fuel_next = max(
            0, self.config.starting_fuel - (next_lap - 1) * self.config.fuel_burn_rate
        )
    ...
    def get_health(
        ...
        _, _, info = self.predict_next_lap(
            driver, current_lap, laps_df, track_condition
        )
